...
export default function Programs() {
  ...
  const programs = [
    ...
    },
    {
      ...
      category: "Big Data",
      topics: [
        "Cloud Architecture Fundamentals & Well-Architected Frameworks",
        "Cloud Networking (AWS VPC, Azure VNet, GCP VPC)",
        "Identity & Access Management (AWS IAM, Azure AD, GCP IAM)",
        "Cloud Security & Compliance",
        "Compute Services (EC2, Azure VM Scale Sets)",
      ...
      <section id="programs-section" className="py-24 bg-transparent relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-20">
            ...
            <h3 className="text-2xl font-bold mb-10 tracking-tight text-white border-l-4 border-ai-cyan pl-4">
              ...
              {programs
                ...
                .map((program) => (
                  <div key={program.title} className="relative group">
                    ...
                    <Card
                      className={`h-full border backdrop-blur-md p-8 flex flex-col justify-between transition-all duration-500 rounded-2xl bg-white/[0.01] hover:shadow-[0_15px_30px_rgba(0,210,255,0.08)] ${
                        program.featured
                          ? "border-ai-cyan/40 hover:border-ai-cyan"
                          : "border-white/10 hover:border-ai-purple/50"
                      }`}
                    >

                      <div className="space-y-5">

                        <span className="inline-block bg-white/5 border border-white/10 text-ai-cyan px-3 py-1 rounded-full text-[10px] font-mono font-bold uppercase">
                          {program.level}
                        </span>

                        <h4 className="text-2xl font-bold text-white tracking-tight group-hover:text-ai-cyan transition-colors">
                          {program.title}
                        </h4>
                        ...
                        <div className="flex gap-4 border-y border-white/5 py-4 text-xs text-gray-300 font-medium">
                          ...
                          <div className="flex items-center gap-1.5">
                            <Clock size={14} className="text-ai-cyan" />
                            {program.duration}
                          </div>

                          <div className="flex items-center gap-1.5">
                            <Zap size={14} className="text-ai-purple" />
                            {program.category}
                          </div>
                        ...
                        <div className="space-y-2.5 pt-2">
                          ...
                          {program.features.map((f) => (
                            ...
                            >
                              <CheckCircle2
                                size={14}
                                className="text-ai-cyan shrink-0"
                              />
                              {f}
                            </div>
                          ))}
                      ...
                      <Button
                        ...
                        className="w-full font-bold mt-8 rounded-xl bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-all cursor-pointer"
                      >
                        Learn More
                      </Button>
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold mb-10 tracking-tight text-white border-l-4 border-ai-purple pl-4">
              ...
              {programs
                ...
                .map((program) => (
                  <div key={program.title} className="relative group">
                    ...
                    <Card
                      className={`h-full border backdrop-blur-md p-8 flex flex-col justify-between transition-all duration-500 rounded-2xl bg-white/[0.01] hover:shadow-[0_15px_30px_rgba(99,102,241,0.08)] ${
                        program.featured
                          ? "border-ai-purple/40 hover:border-ai-purple"
                          : "border-white/10 hover:border-ai-cyan/30"
                      }`}
                    >

                      <div className="space-y-5">

                        <span className="inline-block bg-white/5 border border-white/10 text-ai-purple px-3 py-1 rounded-full text-[10px] font-mono font-bold uppercase">
                          {program.level}
                        </span>

                        <h4 className="text-2xl font-bold text-white tracking-tight group-hover:text-ai-purple transition-colors">
                          {program.title}
                        </h4>
                        ...
                        <div className="space-y-2.5 pt-2">
                          ...
                          {program.features.map((f) => (
                            ...
                            >
                              <CheckCircle2
                                size={14}
                                className="text-ai-purple shrink-0"
                              />
                              {f}
                            </div>
                          ))}
          ...
          <Card className="w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-[#070a13] border border-white/10 p-8 rounded-2xl relative space-y-6 text-gray-200">
            ...
            <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-white/[0.02] border border-white/5 text-sm font-medium">
              ...
              <div>
                ...
                <p className="text-ai-cyan font-bold mt-0.5">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 text-white font-bold">
                ...
                <h3>Syllabus Domains Covered</h3>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-[30vh] overflow-y-auto pr-1">

                {selectedProgram.topics.map(
                  (topic: string, i: number) => (
                    <div
                      key={i}
                      className="flex items-start gap-2.5 p-3 rounded-lg bg-white/[0.01] border border-white/5 text-xs text-gray-300 font-medium"
                    >
                      <CheckCircle2
                        size={14}
                        className="text-ai-cyan shrink-0 mt-0.5"
                      />
            ...
            <div className="flex gap-4 pt-2">
              ...
              <Link
                href="/career-roadmap"
                className="flex-1 py-3 text-center rounded-xl bg-gradient-to-r from-ai-cyan to-ai-purple text-gray-950 font-bold text-sm shadow-md hover:opacity-90 transition-all"
              >
                Apply & Build Roadmap
              </Link>
              ...
              <Button
                variant="outline"
                onClick={() => setSelectedProgram(null)}
                className="flex-1 py-3 font-bold text-sm rounded-xl border-white/10 text-white hover:bg-white/5 cursor-pointer"
              >
                Close Overview
              </Button>
